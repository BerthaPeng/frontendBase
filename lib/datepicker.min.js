...
function(e) {
    var t = function(t, i) {
        if (this.element = e(t), this.format = s.parseFormat(i.format || this.element.data("date-format") || "mm/dd/yyyy"), this.picker = e(s.template).appendTo("body").on({
        ...
        }) : this.component ? this.component.on("click", e.proxy(this.show, this)) : this.element.on("click", e.proxy(this.show, this)), this.minViewMode = i.minViewMode || this.element.data("date-minviewmode") || 0, "string" == typeof this.minViewMode) switch (this.minViewMode) {
        case "months":
            this.minViewMode = 1;
            break;
        case "years":
    ...
    t.prototype = {
        ...
        show: function(t) {
            ...
            function(t) {
                0 == e(t.target).closest(".datepicker").length && s.hide()
            }),
        ...
        update: function(e) {
            this.date = s.parseDate("string" == typeof e ? e: this.isInput ? this.element.prop("value") : this.element.data("date"), this.format),
            this.viewDate = new Date(this.date.getFullYear(), this.date.getMonth(), 1, 0, 0, 0, 0),
            this.fill()
        },
        fillDow: function() {
            for (var e = this.weekStart,
            t = "<tr>"; e < this.weekStart + 7;) t += '<th class="dow">' + s.dates.daysMin[e++%7] + "</th>";
            t += "</tr>",
            this.picker.find(".datepicker-days thead").append(t)
        },
        fillMonths: function() {
            for (var e = "",
            t = 0; 12 > t;) e += '<span class="month">' + s.dates.monthsShort[t++] + "</span>";
        ...
        fill: function() {
            ...
            i = e.getMonth(),
            a = this.date.valueOf();
            this.picker.find(".datepicker-days th:eq(1)").text(s.dates.months[i] + " " + t);
            var n = new Date(t, i - 1, 28, 0, 0, 0, 0),
            h = s.getDaysInMonth(n.getFullYear(), n.getMonth());
            n.setDate(h),
            n.setDate(h - (n.getDay() - this.weekStart + 7) % 7);
            var o = new Date(n);
            o.setDate(o.getDate() + 42),
            o = o.valueOf();
            for (var d, r, l, c = []; n.valueOf() < o;) n.getDay() === this.weekStart && c.push("<tr>"),
            d = this.onRender(n),
            r = n.getFullYear(),
            l = n.getMonth(),
            i > l && r === t || t > r ? d += " old": (l > i && r === t || r > t) && (d += " new"),
            n.valueOf() === a && (d += " active"),
            ...
            p === t && m.eq(this.date.getMonth()).addClass("active"),
            c = "",
            t = 10 * parseInt(t / 10, 10);
            var f = this.picker.find(".datepicker-years").find("th:eq(1)").text(t + "-" + (t + 9)).end().find("td");
            t -= 1;
            for (var w = -1; 11 > w; w++) c += '<span class="year' + ( - 1 === w || 10 === w ? " old": "") + (p === t ? " active": "") + '">' + t + "</span>",
            t += 1;
            f.html(c)
        },
        click: function(t) {
            ...
            case "th":
                ...
                case "prev":
                case "next":
                    this.viewDate["set" + s.modes[this.viewMode].navFnc].call(this.viewDate, this.viewDate["get" + s.modes[this.viewMode].navFnc].call(this.viewDate) + s.modes[this.viewMode].navStep * ("prev" === i[0].className ? -1 : 1)),
                    this.fill(),
                    this.set()
                }
                break;
            case "span":
                if (i.is(".month")) {
                    var a = i.parent().find("span").index(i);
                ...
                0 !== this.viewMode && (this.date = new Date(this.viewDate), this.element.trigger({
                    type: "changeDate",
                    date: this.date,
                    viewMode: s.modes[this.viewMode].clsName
                })),
                this.showMode( - 1),
                this.fill(),
                this.set();
            ...
            case "td":
                if (i.is(".day") && !i.is(".disabled")) {
                    ...
                    i.is(".old") ? a -= 1 : i.is(".new") && (a += 1);
                    var n = this.viewDate.getFullYear();
                    this.date = new Date(n, a, h, 0, 0, 0, 0),
                    this.viewDate = new Date(n, a, Math.min(28, h), 0, 0, 0, 0),
                    this.fill(),
                    this.set(),
                    this.element.trigger({
                        type: "changeDate",
        ...
        showMode: function(e) {
            ...
            this.picker.find(">div").hide().filter(".datepicker-" + s.modes[this.viewMode].clsName).show()
        },
        destroy: function() {
    ...
    e.fn.datepicker = function(s, i) {
        return this.each(function() {
            var a = e(this),
            n = a.data("datepicker"),
            h = "object" == typeof s && s;
            n || a.data("datepicker", n = new t(this, e.extend({},
            e.fn.datepicker.defaults, h))),
            "string" == typeof s && n[s](i)
        })
    },
    ...
    var s = {
        ...
        parseFormat: function(e) {
            var t = e.match(/[.\/\-\s].*?/),
            s = e.split(/\W+/);
            if (!t || !s || 0 === s.length) throw new Error("Invalid date format.");
            return {
                separator: t,
                parts: s
            }
        ...
        parseDate: function(e, t) {
            var s, i = e.split(t.separator),
            e = new Date;
            if (e.setHours(0), e.setMinutes(0), e.setSeconds(0), e.setMilliseconds(0), i.length === t.parts.length) {
                for (var a = e.getFullYear(), n = e.getDate(), h = e.getMonth(), o = 0, d = t.parts.length; d > o; o++) switch (s = parseInt(i[o], 10) || 1, t.parts[o]) {
                case "dd":
                case "d":
                    n = s,
                    e.setDate(s);
        ...
        formatDate: function(e, t) {
            ...
            s.dd = (s.d < 10 ? "0": "") + s.d,
            s.mm = (s.m < 10 ? "0": "") + s.m;
            for (var e = [], i = 0, a = t.parts.length; a > i; i++) e.push(s[t.parts[i]]);
